...
export async function POST(req: NextRequest) {
  ...
  let subject: string;
  if (typeof repository === 'object' && repository !== null && 'full_name' in repository && typeof (repository as Record<string, unknown>).full_name === 'string') {
    subject = (repository as Record<string, string>).full_name;
  } else if (typeof organization === 'object' && organization !== null && 'login' in organization && typeof (organization as Record<string, unknown>).login === 'string') {
    subject = (organization as Record<string, string>).login;
  } else {
    subject = 'unknown';
  }
