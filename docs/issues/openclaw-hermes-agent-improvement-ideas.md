...
- docker / ssh backend の append 未対応、remote backend での `readFile` 系統合、network policy / allowed paths の強制、生成スキル実行の backend 側移譲は引き続き残課題（次の小分け対象）。
...
- 新規 / 追加テスト: `agentDagRunner`（toolsets 永続化・retry 引き継ぎ）、`dashboardScheduleUpdate`（PATCH の toolsets 更新・検証・クリア）、`agentCommands`（承認者 allowlist の後方互換・許可・拒否）。
- `npm test` で Jest 全体（44 スイート / 748 件）の通過、`npx tsc --noEmit` の型チェック通過を確認した。
